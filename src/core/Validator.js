const Validator={
    ...
    tests:[
        ...
        {name:"SPH pair-force momentum symmetry",run(){const a=new PhysicalBody("a","GAS_CLOUD",2,1,new Vec3(-1,0,0),new Vec3(1,0,0)),b=new PhysicalBody("b","GAS_CLOUD",3,1,new Vec3(1,0,0),new Vec3(-1,0,0));const oldH=FluidEngine.h;FluidEngine.h=4;const acc=FluidEngine.computeAccelerations([a,b]),aa=acc.get(a),bb=acc.get(b),res=aa.mult(a.mass).add(bb.mult(b.mass)).mag()/Math.max(aa.mult(a.mass).mag()+bb.mult(b.mass).mag(),1);FluidEngine.h=oldH;return Validator._assert(this.name,res<1e-14,{error:res,tolerance:"pairwise internal SPH force momentum residual <1e-14"});}},{name:"SPH density symmetry",run(){const a=new PhysicalBody("a","GAS_CLOUD",1,1,new Vec3(-1,0,0),new Vec3()),b=new PhysicalBody("b","GAS_CLOUD",1,1,new Vec3(1,0,0),new Vec3());FluidEngine.h=4;FluidEngine.computeState([a,b]);const e=Math.abs(a.sph_density-b.sph_density)/Math.max(a.sph_density,b.sph_density);return Validator._assert(this.name,e<1e-14,{error:e,tolerance:"<1e-14"});}},
        ...
        {name:"Yoshida4 symplectic Kepler bounded energy",run(){
            const run=(method)=>{
                ...
                const o=[
                    new PhysicalBody("S","STAR",M,6.96e8,new Vec3(-cr,0,0),new Vec3(0,-cv,0)),
                    new PhysicalBody("P","PLANET",m,6.371e6,new Vec3(r-cr,0,0),new Vec3(0,v-cv,0))
                ];
                const E0=Diagnostics.getKineticEnergy(o)+Diagnostics.getPotentialEnergy(o),T=2*Math.PI*Math.sqrt(r**3/mu);
                let max=0;
                for(let k=0;k<5000;k++){
                    Integrators[method](o, T/5000, z=>GravityEngine.computeAccelerations(z,false));
                    const E=Diagnostics.getKineticEnergy(o)+Diagnostics.getPotentialEnergy(o);
                    max=Math.max(max,Math.abs((E-E0)/Math.max(Math.abs(E0),1)));
                }
                return max;
            };
            const e=run("yoshida4");
            return Validator._assert(this.name,Number.isFinite(e)&&e<1e-7,{error:e,tolerance:"<1e-7 relative energy envelope over 1 orbit"});
        }},
        {name:"Adaptive RK4 finite-error control",run(){
            ...
            const B=new PhysicalBody("P","PLANET",m,6.371e6,new Vec3(r-cr,0,0),new Vec3(0,vc-cv,0));
            const o=[A,B],T=2*Math.PI*Math.sqrt(r**3/mu);
            let t=0,maxErr=0,steps=0;
            while(t<T){
                const rem=Math.min(86400,T-t);
                const q=Integrators.rk4Adaptive(o,rem,z=>GravityEngine.computeAccelerations(z,false),1e-9,1e-3,10);
                if(!(q.acceptedDt>0&&q.acceptedDt<=rem&&Number.isFinite(q.error))) return Validator._assert(this.name,false,{error:Infinity,tolerance:"finite accepted dt/error"});
        ...
        {name:"Astronomy Engine ephemeris adapter",run(){
            if(typeof EphemerisEngine==="undefined" || !EphemerisEngine.available()) return Validator._assert(this.name,false,{error:Infinity,tolerance:"provider available"});
            const s=EphemerisEngine.solarSystem(0);
            const finite=s.length===10&&s.every(x=>[x.pos.x,x.pos.y,x.pos.z,x.vel.x,x.vel.y,x.vel.z].every(Number.isFinite));
            const earth=s.find(x=>x.name==="Earth");
            const range=earth ? Math.abs(earth.pos.mag()/PhysicsConstants.AU-1) : Infinity;
            return Validator._assert(this.name,finite&&range<.03,{error:Math.max(finite?0:1,range),tolerance:"finite 10-body state; Earth radius within 3% of 1 AU"});
        }},
    ],
