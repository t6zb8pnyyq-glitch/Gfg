const MHD1D={
    ...
    step(U,dx,dt,Bx,cfl=.4){
        ...
        const F=[];for(let i=0;i<U.length-1;i++)F[i]=this.rusanovFlux(U[i],U[i+1],Bx);
        const out=U.map(u=>u.slice());
        for(let i=1;i<U.length-1;i++){for(let k=0;k<7;k++)out[i][k]-=dt/dx*(F[i][k]-F[i-1][k]);this.flux(out[i],Bx);}
        return out;
    }
};